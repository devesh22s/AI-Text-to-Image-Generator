...
const PlanIcon = () => (
  <svg
    viewBox="0 0 24 24"
    className="w-12 h-12 mb-4 drop-shadow-[0_0_8px_rgba(212,175,55,0.6)]"
  >
    <defs>
  ...
  const initPay = async (order) => {
    const options = {
      ...
      handler: async (response) => {
        ...
        try {
          const {data} = await axios.post(backendurl + '/api/user/verifyRazor', response, {headers :{token}})
          if(data.success){
            loadCreditData();
            navigate('/')
    ...
    const razp = new window.Razorpay(options)
    razp.open()
  };
    ...
    <div className="min-h-[80vh] text-center pt-14 mb-10 px-4">
      ...
      <div className="flex flex-wrap justify-center gap-8 text-left max-w-6xl mx-auto">
        {plans.map((item, index) => (
          <div
            key={index}
            className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8 text-gray-300 hover:scale-105 hover:border-cyan-500/50 hover:shadow-[0_10px_30px_rgba(6,182,212,0.15)] transition-all duration-500 w-full max-w-[300px] flex flex-col"
          >
            <PlanIcon />
            ...
            <div className="mt-auto">
              ...
              <button
                ...
                className={`w-full text-white font-semibold mt-2 text-base rounded-xl py-3.5 transition-all ${
                  user
